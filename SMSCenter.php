...
/**
 * Библиотека для работы с сервисом SMSЦентр.
 *
 * Позволяет отправлять все доступные типы сообщений и получать: стоимость сообщений, баланс, статус, информацию об операторе.
 ...
 *	));
 *
 *	$sms->getStatus('+79081234568', 6, SMSCenter::STATUS_INFO_EXT);
 *	$sms->getOperatorInfo('79081234568');
 *	$sms->sendMessage('+79081234568', 'Привет!', 'SuperIvan');
 *	$sms->getCost('+79081234568', 'Начало около 251 млн лет, конец — 201 млн лет назад, длительность около 50 млн лет. До наступления триаса все материки существовали в виде единого гигантского суперматерика — Пангеи. С наступлением триаса Пангея начала постепенно раскалываться.');
 *	$sms->getBalance();
 * </pre>
 *
 * @version 0.1
